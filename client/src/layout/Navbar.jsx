...
export default function Navbar({ onLogoClick, activeNav: propsActiveNav, onNavClick, onLogout }) {
    const { user } = useContext(Context);
    const [currentDate, setCurrentDate] = useState('');
    const [isProfileOpen, setIsProfileOpen] = useState(false);
    const [isNotificationsOpen, setIsNotificationsOpen] = useState(false);
    const [internalActiveNav, setInternalActiveNav] = useState('Contests');

    const activeNav = propsActiveNav !== undefined ? propsActiveNav : internalActiveNav;
    ...
    const navItems = ['Challenges', 'Practice', 'Contests'];
        ...
        <header className="h-16 border-b border-slate-200 dark:border-white/10 bg-background-light dark:bg-bg-dark sticky top-0 z-50 w-full">
            <div className="w-full mx-auto px-6 h-full flex items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center gap-6">
                        {navItems.map((item) => (
                            <button
                                ...
                                className={`text-sm font-medium transition-colors pb-1 ${activeNav === item
                ...
                <div className="flex items-center gap-4">
                        ...
                        {isNotificationsOpen && (
                            <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-white/10 py-2 z-50">
                                ...
                                {notifications.map((notif) => (
                                    <button
                                        key={notif.id}
                                        onClick={() => console.log(`Notification ${notif.id} clicked`)}
                    ...
                    {/* Current Date */}
                    <button
                        onClick={() => console.log('Date clicked:', currentDate)}
                        className="hidden lg:block text-sm text-slate-500 dark:text-white/50 hover:text-slate-700 dark:hover:text-white transition-colors px-3 py-1 rounded-md hover:bg-slate-100 dark:hover:bg-white/5"
                    >
                        {currentDate}
                    </button>
